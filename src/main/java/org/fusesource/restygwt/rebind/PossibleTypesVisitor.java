    ...
    protected List<Subtype> getPossibleTypesForOther(GeneratorContext context, JClassType classType,
        ...
        if (typeResolver != null) {
            ...
            if (RestyJsonTypeIdResolver.class.isAssignableFrom(resolverClass)) {
                ...
                restyResolver =
                    JsonEncoderDecoderClassCreator.getRestyResolverClassMap(context, logger).get(resolverClass);
                if (restyResolver == null) {
                    logger.log(BaseSourceCreator.ERROR, "Could not find RestyJsonTypeIdResolver for " + resolverClass +
                        " did you forget to put <extend-configuration-property name=\"org.fusesource.restygwt" +
                        ".jsontypeidresolver\" " +
                        "value=\"<fully-qualified-class-implementing-RestyJsonTypeIdResolver>\"/> in your *.gwt.xml?");
                    throw new UnableToCompleteException();
                }
            }
            ...
            for (JsonSubTypes.Type type : types) {
                if (type.name() != null && !type.name().isEmpty()) {
                    ...
                    if (nameAnnotation == null || nameAnnotation.value() == null || nameAnnotation.value().isEmpty()) {
                        logger.log(BaseSourceCreator.ERROR,
                            "Cannot find @JsonTypeName annotation for type: " + type.value());
                        throw new UnableToCompleteException();
                    }
                    JClassType typeClass = BaseSourceCreator.find(type.value(), logger, context);
                    if (!isLeaf || classType.equals(typeClass)) {
